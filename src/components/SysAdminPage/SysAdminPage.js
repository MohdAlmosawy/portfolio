...
const SysAdminPage = () => {
  ...
  const getFileContent = (path, fileName, options = '') => {
    ...
    for (const dir of path.split('/').slice(1)) {
  ...
  const displayWelcomeMessage = useCallback(() => {
    // Check screen width
    const isLargeScreen = window.innerWidth >= 768; // md breakpoint in Tailwind

    terminalInstance.current.writeln(''); // Add initial new line

    if (isLargeScreen) {
      // Display ASCII art on large screens
      ...
      // Display simple text message on small screens
      terminalInstance.current.writeln("\x1b[1m=== Welcome to the Terminal ===\x1b[0m"); // Bold text
      terminalInstance.current.writeln("");
      terminalInstance.current.writeln("The World of Sayed as a SysAdmin");
      terminalInstance.current.writeln("");
    }
